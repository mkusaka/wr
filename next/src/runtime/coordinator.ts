...
export class CoordinatorBridge {
    ...
    async guidance(): Promise<string> {
        ...
        return [
            ...
            this.context.runtime === "claude" ? "A read-only foreground Claude child requires an explicit assignment: run wr-next delegate REF --read-only, then put the returned spawnDirective on the first line of exactly one Agent prompt. Nested/background children are denied." : this.context.runtime === "omp" ? "Each OMP task child requires a distinct --read-only delegation directive on the first line of its task. Only bound inspection tools, bounded wr-next/Git shell commands, and native hub conversations are available; write-capable and nested native tools are denied." : "Native subagents are not safely bindable from this runtime's current spawn-correlation profile; use explicit delegated runs. Never reuse this root's context.",
