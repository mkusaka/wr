...
export class Tokens {
    ...
    read(token: string): Principal {
        ...
        const x = object(JSON.parse(Buffer.from(payload, "base64url").toString()), ["id", "device", "role", "execution", "checks", "generation", "runtimeRoot", "runtimeAgent", "workspace", "expires"]);
        ...
        return { id: text(x.id, "principal"), device: text(x.device, "device"), role: choice(x.role, ["operator", "worker", "launcher", "collector", "adapter"] as const), ...(x.execution ? { execution: text(x.execution, "execution") } : {}), ...(x.checks ? { checks: list(x.checks, v => text(v, "check")) } : {}), ...(x.generation !== undefined ? { generation: integer(x.generation, "generation", 1) } : {}), ...(x.runtimeRoot ? { runtimeRoot: text(x.runtimeRoot, "runtimeRoot") } : {}), ...(x.runtimeAgent ? { runtimeAgent: text(x.runtimeAgent, "runtimeAgent") } : {}) };
    }
}
