...
import { coordinatorFor } from "../domain/coordination.js";
...
export function runtimeView(s: State, p: Principal) {
    ...
    const scope = co ? descendants(s, co.work) : p.role === "operator" || p.role === "adapter" ? null : descendants(s, boundExecution(s, p).work);
    if (p.role === "adapter")
        adapterAgent(s, p, p.runtimeRoot ?? "");
    const allowed = values(s.runtimeAgents).filter(a => p.role === "operator" || p.role === "adapter" && a.root === p.runtimeRoot || co && a.root === co.runtimeAgent || scope && a.execution && scope.has(s.executions[a.execution]!.work));
    ...
    const nodes = allowed.sort((a, b) => a.id.localeCompare(b.id)).map(a => {
        ...
        const coordinator = values(s.coordinators).some(c => c.runtimeAgent === a.id);
        const childrenActive = values(s.runtimeAgents).filter(c => c.parent === a.id && c.state !== "ended").length;
        return { id: a.id, parent: a.parent && ids.has(a.parent) ? a.parent : null, runtime: a.runtime, session: a.externalSessionId, agentId: a.externalAgentId, invocation: a.invocationId, run: a.run, execution: a.execution, work: e ? s.work[e.work]!.key : null, delegation: e?.delegation ?? null, continuedFrom: e?.continuedFrom ?? null, state: a.state, orphan: !!parent && parent.state === "ended" && a.state !== "ended", coordinator, unassigned: !e && !coordinator, childrenActive, endedAt: a.endedAt };
    });
...
export function runtimeMermaid(v: ReturnType<typeof runtimeView>): string {
    ...
    for (const n of v.nodes)
        lines.push(`    ${ids.get(n.id)}["${esc(`${n.runtime}:${n.agentId} / ${n.work ?? (n.coordinator ? "coordinator" : "unassigned")} / ${n.state}${n.orphan ? " / orphan" : ""}`)}"]`);
    for (const n of v.nodes)
        if (n.parent) {
