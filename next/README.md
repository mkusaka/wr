...
Installation, actual loading/trust, and native subagent binding are separate statuses. `init` does not claim all three are equally ready. Claude supports one explicitly delegated foreground read-only child at a time. Stock Codex 0.153.4 passed the integrated, explicitly delegated MAv1 read-only child smoke with deterministic local model responses and native messaging. OMP 18.1.13 supports explicitly delegated concurrent read-only native task children with native hub conversation. See [repository integrations](docs/repository-integrations.md) for required Codex settings, worktree handling, and runtime limitations.
...
Claude and Codex use their permanent project hooks; OMP uses its native extension. The adapters cover root lifecycle/startup guidance and targeted PR observations. Claude can bind one foreground read-only child after `wr-next delegate REF --read-only`; put the returned `spawnDirective` on the first line of the Agent prompt. Codex MAv1 can bind an explicitly delegated read-only child for each supported spawn when the directive is the first line of exactly one `spawn_agent`, `Agent`, or `multi_agent_v1spawn_agent` message; child binding waits at most 3 seconds for its exact receipt and otherwise denies the child without inheriting root context. Its root may natively spawn and use verified same-root `send_input`/`wait_agent` peers without a Work claim. OMP 18.1.13 consumes one directive per native `task.tasks[].task` and preserves root/child and sibling `hub` conversation. MAv2, nested, writable, close/resume, unmarked, and background native spawning remains denied rather than attributed to the parent. Devin is wrapper-only. Generic explicit subprocess execution remains available with `--runtime generic`, but does not replace native messaging.
